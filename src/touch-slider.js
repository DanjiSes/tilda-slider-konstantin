...
jScroll.prototype.init = function () {
  ...
  if (this.$obj.length == 1) {
    ...
    this.$prev = $(`.sh-sliderprev-${this.$obj.parents(".t-rec").attr("id")}`);
    this.$next = $(`.sh-slidernext-${this.$obj.parents(".t-rec").attr("id")}`);
...
jScroll.prototype.bindEvent = function () {
  ...
  if (window.outerWidth <= 640)
    return _this.$obj.attr("style", "overflow: auto !important;");
